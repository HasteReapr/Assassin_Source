...
namespace AssassinMod.Characters.Entities.Decoy
{
    ...
    public abstract class DecoyBase
    {
        ...
        public virtual void CreatePrefab()
        {
            if (!local)
            {
                prefab = PrefabAPI.InstantiateClone(AssassinPlr.AssassinPrefab, CloneName + "Body");
                //prefab = PrefabAPI.InstantiateClone(UnityEngine.AddressableAssets.Addressables.LoadAssetAsync<GameObject>("AssassinAssets").WaitForCompletion(), "AssassinSurvivorBody");
            }
            if (!localMaster)
            {
        ...
        public void SetupModel(GameObject prefab, GameObject model, float turnSpeed = 1200f)
        {
            ...
            GameObject modelbase = new GameObject("Model Base");
